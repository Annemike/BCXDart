...
from sbp.observation import SBP_MSG_OBS, SBP_MSG_BASE_POS_LLH, SBP_MSG_EPHEMERIS_GPS
...
class GroupServer(Server):
    ...
    def _copy_from_parameter_server(self):
        ...
        for parents, param in self._extract_param_tree(self.type.config_description):
            try:
                self.config[self._get_config_name(parents, param['name'])] = rospy.get_param(
                    self._get_param_name(parents, param['name']))
            except KeyError:
                pass

    def _copy_to_parameter_server(self):
        for parents, param in self._extract_param_tree(self.type.config_description):
            rospy.set_param(self._get_param_name(parents, param['name']),
                            self.config[self._get_config_name(parents, param['name'])])
...
def ros_time_from_sbp_time(msg):
    return rospy.Time(GPS_EPOCH + msg.wn * WEEK_SECONDS + msg.tow * 1E-3 + msg.ns_residual * 1E-9)
...
FIX_MODE = {0: "Invalid [0]", 1: "Single Point Position (SPP) [1]", 2: "Differential GNSS [2]", 3: "Float RTK [3]", 4: "Fixed RTK [4]",-1: "No fix"}
...
class PiksiROS(object):
    def __init__(self):
        ...
        self.heartbeat_diag = diagnostic_updater.FrequencyStatus(
            diagnostic_updater.FrequencyStatusParam({'min': self.diag_heartbeat_freq, 'max': self.diag_heartbeat_freq},
    ...
    def setup_comms(self):
        ...
        if self.obs_serial_recv:
            self.obs_receivers.append(
                SerialReceiver(self.obs_serial_port, self.obs_serial_baud_rate, self.callback_external))
    ...
    def setup_pubsub(self):
        ...
        freq_params = diagnostic_updater.FrequencyStatusParam(
            {'min': self.diag_update_freq, 'max': self.diag_update_freq}, self.diag_freq_tolerance,
            self.diag_window_size)
        ...
        self.pub_spp_fix = diagnostic_updater.DiagnosedPublisher(
            rospy.Publisher("~spp_fix", NavSatFix, queue_size=1000), self.diag_updater, freq_params, time_params)

        self.pub_rtk_fix = diagnostic_updater.DiagnosedPublisher(
            rospy.Publisher("~rtk_fix", NavSatFix, queue_size=1000), self.diag_updater, freq_params, time_params)
        ...
        self.pub_odom = diagnostic_updater.DiagnosedPublisher(rospy.Publisher("~odom", Odometry, queue_size=1000),
                                                              self.diag_updater, freq_params, time_params)
        self.pub_time = diagnostic_updater.DiagnosedPublisher(rospy.Publisher("~time", TimeReference, queue_size=1000),
                                                              self.diag_updater, freq_params, time_params)
    ...
    def piksi_start(self):
        ...
        self.dr_srv = GroupServer(PiksiDriverConfig, self.reconfig_callback, ns_prefix=('dynamic_reconfigure',),
                                  nest_groups=False)
        ...
        self.piksi.add_callback(self.callback_sbp_base_pos, msg_type=SBP_MSG_BASE_POS_LLH)
        if self.publish_ephemeris:
...
#TODO
        if msg.flags & 0x07:
            ...
            out.position_covariance[0] = msg.h_accuracy ** 2 #self.rtk_h_accuracy ** 2
            out.position_covariance[4] = msg.h_accuracy ** 2 #self.rtk_h_accuracy ** 2
            out.position_covariance[8] = msg.v_accuracy ** 2 #self.rtk_v_accuracy ** 2
    ...
    def callback_sbp_base_pos(self, msg, **metadata):
        ...
        if self.debug:
            rospy.loginfo("Received SBP_MSG_BASE_POS_LLH (Sender: %d): %s" % (msg.sender, repr(msg)))
    ...
    def diag(self, stat):
        ...
        if self.last_baseline:
            ...
            stat.add("Baseline fix mode", FIX_MODE[self.last_baseline.flags & 0x07])
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            try:
                rospy.loginfo("Connecting to SwiftNav Piksi on %s:%s" % (self.piksi_host,self.piksi_port))
                self.connect_piksi()
